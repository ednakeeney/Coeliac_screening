# Replace Markov update with C/C++

# Uses the Rcpp package
require(Rcpp)
# Compiles the C++ file for the Markov loop
Rcpp::sourceCpp(file = "code/rcpp_loop_full.cpp")


generate_net_benefit_cpp <- function(input_parameters, 
...
#   
   ...
   time_in_states <- matrix(nrow = length(strategies_of_interest), ncol = n_states)
   rownames(time_in_states) <- strategies_of_interest
   colnames(time_in_states) <- state_names
   for(test_name in strategies_of_interest) {
