  ...
  res <- within(list(), {
    ...
    for(i in 1:length(eps.v)) {
     # mlmcres <- mlmc(Lmin, Lmax, N0, eps.v[i], mlmc_l, alpha, beta, gamma2, parallel, ...)
      ...
      mlmc_cost <- c(mlmc_cost, sum(Nl[[i]]*(2^gamma)^(0:l)*cost[1]))
      l2 <- min(l+1, length(var2))
      #std_cost <- c(std_cost, var2[l2]*M^l / ((1.0-theta)*eps.v[i]^2))
      std_cost <- c(std_cost, var2[l2]*(2^gamma)^l*cost[1] / ((1.0-theta)*eps.v[i]^2))
    ...
    rm(l2, mlmcres, i, theta, gamma, L1, L2, check, kurt,  l, sums, cost)
  })
  class(res) <- "mlmc.test"
  res
}
