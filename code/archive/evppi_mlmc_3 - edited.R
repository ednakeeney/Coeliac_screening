...
EVPPI_utilities_std_p<-function(M, N)
{
  ...
  input_parameters_mlmc <- generate_model_parameters(starting_age, population = population,
  ...
  NetB <- generate_NetB(n_samples = NN, input_parameters_mlmc = input_parameters_mlmc)
...
print("Utilities.........................................")
tic()
tst_utilities <- mlmc.test(EVPPI_utilities_l_p, M=2, N=32,
                   L=5, N0=32,
                   eps.v=c(60,30),
                   Lmin=2, Lmax=10)
toc()
evpi_table["Utilities and disutilities", c("Per person", "Population")] <- (m_of_interest$evi[201] - tst_utilities$P[length(tst_utilities$P)]) * c(1, discounted_population_size)
...
tst_rates <- mlmc.test(EVPPI_rates_l_p, M=2, N=128,
                           ...
                           Lmin=2, Lmax=10)
toc()
evpi_table["Rates of osteoporosis and NHL", c("Per person", "Population")] <- (m_of_interest$evi[201] - tst_rates$P[length(tst_rates$P)]) * c(1, discounted_population_size)
write.csv(evpi_table, paste0("results/", population, "/", population, "_evppi_summary.csv"))
...
tst_gfd <- mlmc.test(EVPPI_gfd_l_p, M=2, N=128,
                       ...
                       Lmin=2, Lmax=10)
toc()
evpi_table["GFD effect", c("Per person", "Population")] <- (m_of_interest$evi[201] - tst_gfd$P[length(tst_gfd$P)]) * c(1, discounted_population_size)
write.csv(evpi_table, paste0("results/", population, "/", population, "_evppi_summary.csv"))
...
tst_test_accuracy <- mlmc.test(EVPPI_test_accuracy_l_p, M=2, N=128,
                     ...
                     Lmin=2, Lmax=10)
toc()
evpi_table["Test accuracies", c("Per person", "Population")] <- (m_of_interest$evi[201] - tst_test_accuracy$P[length(tst_test_accuracy$P)]) * c(1, discounted_population_size)
write.csv(evpi_table, paste0("results/", population, "/", population, "_evppi_summary.csv"))
...
tst_late_diagnosis <- mlmc.test(EVPPI_late_diagnosis_l_p, M=2, N=128,
                               ...
                               Lmin=2, Lmax=10)
toc()
evpi_table["Probability of late diagnosis", c("Per person", "Population")] <- (m_of_interest$evi[201] - tst_late_diagnosis$P[length(tst_late_diagnosis$P)]) * c(1, discounted_population_size)
write.csv(evpi_table, paste0("results/", population, "/", population, "_evppi_summary.csv"))
